...
<section className="px-6 sm:px-12 py-12 bg-[#f8fafc]">
  ...
  <ul className="space-y-5 text-base sm:text-lg">
    <li className="flex items-start gap-3">
      <BookOpen className="w-5 h-5 text-green-600 mt-1 shrink-0" />
      <div>
        <span className="font-medium text-gray-600">Methodology:</span>{" "}
        <span className="text-gray-800">{project.methodology}</span>
      </div>
    ...
    <li className="flex items-start gap-3">
      <CalendarDays className="w-5 h-5 text-green-600 mt-1 shrink-0" />
      <div>
        <span className="font-medium text-gray-600">Issuance Year:</span>{" "}
        <span className="text-gray-800">{project.vintageYear}</span>
      </div>
    </li>
    ...
    <li className="flex items-start gap-3">
      ...
      <div>
        ...
        <span className="text-gray-800">{project.retired ? "Yes" : "No"}</span>
      </div>
    </li>

    <li className="flex items-start gap-3">
      <Globe2 className="w-5 h-5 text-green-600 mt-1 shrink-0" />
      <div>
        <span className="font-medium text-gray-600">Registry:</span>{" "}
        <a
    ...
    <li className="flex items-start gap-3">
      ...
      <div>
        ...
        <span className="text-gray-800">
          {project.placeName}, {project.city}, {project.state}, {project.country}
        </span>
      </div>
    </li>
  </ul>
  ...
  <div>
    ...
    <div className="mt-6 space-y-4 text-gray-800 text-base sm:text-lg">
      <div className="flex justify-between items-start border-b pb-2">
        ...
        <span className="text-green-800 font-bold text-lg">${project.pricePerTon}</span>
      </div>

      <div className="flex justify-between items-start border-b pb-2">
        <span className="text-gray-600 font-medium">📦 Total Tons Available</span>
        <span className="font-semibold">{project.tons}</span>
      </div>

      <div className="flex justify-between items-start">
        <span className="text-gray-600 font-medium">🧾 Total Project Cost</span>
        <span className="font-semibold text-green-900 text-lg">${project.totalPrice}</span>
      </div>
    </div>
  </div>
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
        {project.sdgs.map((goal, idx) => (
          <div
            key={idx}
            className="bg-white border border-green-100 rounded-xl p-4 shadow-sm hover:shadow-md transition duration-200"
          >
...
{project.additionalNotes && (
  ...
  <div className="max-w-4xl mx-auto">
    ...
    <button
      ...
      className="inline-block bg-white text-green-700 hover:bg-green-100 font-semibold py-3 px-8 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 text-base sm:text-lg"
    >
      🌱 Buy Carbon Credit
    </button>
