...
const planOrder = ["plan_1", "plan_2", "plan_3"];
  ...
  useEffect(() => {
    const fetchPricing = async () => {
      try {
        // Updated endpoint to individual
        const response = await fetch(`${API_BASE_URL}/pricing/individual`);
        if (!response.ok) {
...
const isCurrent =
  ...
  (
    ...
    (currentDuration === "monthly" && billing === "monthly")
  );
...
const isUpgrade = isHigher || isSamePlanUpgrade;
            ...
            return (
              ...
              <button
  disabled={isCurrent || isLower || (isSamePlan && currentDuration === "yearly")}
  onClick={() => {
